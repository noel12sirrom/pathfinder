...
def write_prolog_file(roads):
    ...
    try:
        ...
        for i, line in enumerate(lines):
            if '% Road network' in line or '% ----------------------' in line:
                if road_start_idx is None:
                    road_start_idx = i
                elif road_start_idx is not None and road_end_idx is None:
                    # Check if this is the end marker
                    if i > road_start_idx + 5:  # Give some room for header
                        road_end_idx = i
                        break
        ...
        if road_end_idx is None:
            ...
            for i in range(len(lines) - 1, -1, -1):
                if lines[i].strip().startswith('road('):
                    road_end_idx = i + 1
                    break
                elif lines[i].strip().startswith('%') and 'edge_info' in lines[i]:
                    road_end_idx = i
                    break
